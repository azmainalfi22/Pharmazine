...
def require_permission(permission: Permission):
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            # Get user roles (primary source: Profile.role as single role)
            user_roles = []
...
def get_current_user_permissions(db: Session, user_id: str) -> Dict:
    ...
    Note: Import Profile and UserRole here to avoid circular import
    """
    # Lazy import to avoid circular dependency
    from main import Profile, get_roles_for_user
    
    user = db.query(Profile).filter(Profile.id == user_id).first()
    if not user:
        return {"permissions": [], "roles": []}
    
    # Get user roles using central resolver (reads Supabase live, falls back local)
