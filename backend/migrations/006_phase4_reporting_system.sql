...
SELECT 
    ...
    SUM(CASE WHEN s.payment_method = 'card' THEN s.net_amount ELSE 0 END) as card_sales,
    SUM(CASE WHEN s.payment_method = 'upi' THEN s.net_amount ELSE 0 END) as upi_sales,
...
SELECT 
    ...
    s.created_at::DATE + (COALESCE(c.payment_deadline_days, 0) || ' days')::INTERVAL as due_date,
    CASE
        WHEN s.created_at::DATE + (COALESCE(c.payment_deadline_days, 0) || ' days')::INTERVAL < CURRENT_DATE 
        THEN (CURRENT_DATE - (s.created_at::DATE + (COALESCE(c.payment_deadline_days, 0) || ' days')::INTERVAL))::INTEGER
        ELSE 0
    END as days_overdue
FROM sales s
LEFT JOIN customers c ON s.customer_name = c.name OR s.customer_phone = c.contact
...
SELECT 
    ...
    SUM(CASE WHEN payment_method = 'card' THEN net_amount ELSE 0 END) as card_sales,
    SUM(CASE WHEN payment_method = 'upi' THEN net_amount ELSE 0 END) as upi_sales
FROM sales
