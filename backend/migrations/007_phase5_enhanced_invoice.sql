...
-- ============================================
ALTER TABLE IF NOT EXISTS sales_items
...
GROUP BY s.id, c.name, c.contact, c.email, c.address, c.tax_number, u.full_name;
...
SELECT 
    ...
    SUM(CASE WHEN payment_method = 'upi' THEN grand_total ELSE 0 END) as upi_sales,
    AVG(grand_total) as average_bill_value,
    MAX(grand_total) as highest_bill,
    MIN(grand_total) as lowest_bill
FROM sales
