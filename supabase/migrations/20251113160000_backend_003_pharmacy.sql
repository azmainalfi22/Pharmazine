...
-- Insert default alert settings (idempotent without requiring a unique constraint)
INSERT INTO expiry_alert_settings (alert_days_before, alert_level, notification_method)
SELECT 90, 'info', ARRAY['system']::text[]
WHERE NOT EXISTS (SELECT 1 FROM expiry_alert_settings WHERE alert_days_before = 90 AND alert_level = 'info');
INSERT INTO expiry_alert_settings (alert_days_before, alert_level, notification_method)
SELECT 60, 'warning', ARRAY['system', 'email']::text[]
WHERE NOT EXISTS (SELECT 1 FROM expiry_alert_settings WHERE alert_days_before = 60 AND alert_level = 'warning');
INSERT INTO expiry_alert_settings (alert_days_before, alert_level, notification_method)
SELECT 30, 'critical', ARRAY['system', 'email', 'sms']::text[]
WHERE NOT EXISTS (SELECT 1 FROM expiry_alert_settings WHERE alert_days_before = 30 AND alert_level = 'critical');
