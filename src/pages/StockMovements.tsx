...
export default function StockMovements() {
  ...
  const loadMovements = async () => {
    try {
      setLoading(true);
      logger.debug("Loading stock movements from:", `${API_CONFIG.API_ROOT}/stock-transactions`);
      const response = await fetch(`${API_CONFIG.API_ROOT}/stock-transactions`, {
        headers: getAuthHeaders(),
      });
      logger.debug("Stock movements response:", response.status);
      if (response.ok) {
        const data = await response.json();
        logger.debug("Stock movements data:", data);
