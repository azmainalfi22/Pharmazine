...
const SalesReturnPage = () => {
  ...
  const handleAddItem = () => {
    ...
    setReturnItems((prevItems) => [...prevItems, item]);
  ...
  const processReturn = async () => {
    if (returnItems.length === 0) {
      toast.error('Please add items to process return');
      return;
    }
    ...
    try {
      for (const item of returnItems) {
        await apiClient.createStockTransaction({
          ...
          unit_price: 0, // Sales return doesn't affect cost
          reference_id: selectedSale || null,
          notes: `Return reason: ${item.reason}`,
        });
      }
    ...
    <div className="space-y-8 p-8">
                ...
                <SelectContent>
                  {sales.map((sale) => (
                    <SelectItem key={sale.id} value={sale.id}>
                      {sale.customer_name} - ৳{sale.net_amount.toLocaleString()}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
      ...
      <div className="bg-white border rounded-lg overflow-hidden mb-12">
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-gray-700 hover:bg-gray-800">
                ...
                <TableHead className="text-white font-semibold text-right">Return Reason</TableHead>
        ...
        <div className="bg-gray-50 p-4 flex justify-end gap-3">
          ...
          <Button onClick={processReturn} className="bg-green-600 hover:bg-green-700 text-white">
            Process Return
          </Button>
        </div>
      </div>
