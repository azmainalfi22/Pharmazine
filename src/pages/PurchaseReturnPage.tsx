    ...
    try {
      for (const item of returnItems) {
        await apiClient.createStockTransaction({
          ...
          unit_price: 0,
          notes: `Purchase Return to ${suppliers.find(s => s.id === selectedSupplier)?.name}. ${item.reason}: ${item.notes}`,
          reason: item.reason
        });
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-teal-600 via-emerald-600 to-teal-700 p-8 rounded-2xl border-2 border-teal-200/20 shadow-2xl">
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-3xl font-bold text-white drop-shadow-lg mb-2">Purchase Return</h1>
            <p className="text-white/90 text-base">Return purchased items to suppliers</p>
          </div>
          <Button 
            variant="outline" 
            onClick={fetchData} 
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-teal-200 dark:border-teal-800 hover:bg-teal-50 dark:hover:bg-teal-950/20">
                ...
                <TableHead className="text-teal-900 dark:text-teal-100 font-semibold text-right">Return Reason</TableHead>
              ...
              <TableRow>
                ...
                <TableCell className="text-right">
                  <Button 
                    ...
                    disabled={!selectedProduct || newItem.quantity <= 0 || !newItem.reason}
      ...
      {returnItems.length > 0 && (
        <div className="bg-card border-2 rounded-lg overflow-hidden mb-12 shadow-lg">
          <div className="bg-gradient-to-r from-teal-50 to-emerald-50 dark:from-teal-950/20 dark:to-emerald-950/20 p-4 border-b-2 border-teal-200 dark:border-teal-800">
            <h2 className="text-lg font-bold text-teal-900 dark:text-teal-100">Items to Return ({returnItems.length})</h2>
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">Return Reason</TableHead>
      ...
      {returnItems.length > 0 && (
        <div className="flex justify-end">
          <Button 
            ...
            className="bg-gradient-to-br from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white shadow-lg hover:shadow-xl transition-all border-0 disabled:opacity-50"
          >
