...
interface Product {
  ...
  is_prescription_required: boolean;
  prescription_required?: boolean; // Alternative field name
...
export default function POSSystem() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (validBatches.length === 0 && data.length > 0) {
          toast.info(
            "All batches for this product are either expired or out of stock"
          );
        }
      }
    } catch (error) {
  ...
  const handleAddToCart = (batch: Batch) => {
    ...
    // CRITICAL: Check prescription requirement
    const requiresPrescription =
      selectedProduct.is_prescription_required ||
      selectedProduct.prescription_required;
    if (requiresPrescription && !customerInfo.phone) {
      toast.error(
        "Prescription medicines require customer information. Please enter customer phone number."
      );
    ...
    if (daysUntilExpiry < 90 && daysUntilExpiry >= 30) {
      toast.info(`Note: Batch expires in ${daysUntilExpiry} days`);
    }
    ...
    if (existingIndex >= 0) {
      ...
      if (newQuantity > batch.quantity_remaining) {
        toast.error(
          `Only ${batch.quantity_remaining} units available in this batch`
        );
        return;
      }

      updated[existingIndex].quantity = newQuantity;
      updated[existingIndex].total_price = calculateItemTotal(
            ...
            ${items
              ...
              <tr>
                <td colspan="4" style="font-size: 10px; color: #666;">Batch: ${
                  item.batch_number
                }, Exp: ${format(new Date(item.expiry_date), "MMM yyyy")}</td>
              </tr>
            `
              )
              .join("")}
                ...
                <ScrollArea className="h-[400px]">
                  <div className="space-y-2">
                    {cart.map((item, index) => (
                      <div
                        key={index}
                        className="p-3 border rounded-lg glass-subtle"
                      >
                ...
                <div>
                  ...
                  {paymentInfo.paid_amount >= totals.grandTotal && (
                    <div className="text-sm mt-1 text-green-600">
                      Change: $
                      {(paymentInfo.paid_amount - totals.grandTotal).toFixed(2)}
                    </div>
                  )}
                </div>
              )}
      ...
      <Dialog open={checkoutDialog} onOpenChange={setCheckoutDialog}>
        <DialogContent className="glass-strong max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Review order details before completing
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 bg-primary/5 rounded-lg border border-primary/20 space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <Badge className="pharmacy-badge capitalize">
                  {paymentInfo.method}
                </Badge>
              </div>
              {paymentInfo.method === "cash" && (
