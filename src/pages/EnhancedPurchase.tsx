...
export default function EnhancedPurchase() {
  ...
  const loadSuppliers = async () => {
    try {
      ...
      logger.debug("Suppliers response status:", response.status);
      if (response.ok) {
        const data = await response.json();
        logger.debug("Suppliers loaded:", data.length);
        setSuppliers(data);
      } else {
        logger.error("Failed to load suppliers, status:", response.status);
      }
    } catch (error) {
  ...
  const loadProducts = async () => {
    try {
      logger.debug("Loading products from:", `${API_CONFIG.API_ROOT}/products`);
      const response = await fetch(`${API_CONFIG.API_ROOT}/products`, {
        headers: getAuthHeaders()
      });
      logger.debug("Products response status:", response.status);
      if (response.ok) {
        const data = await response.json();
        logger.debug("Products loaded:", data.length);
        setProducts(data);
      } else {
