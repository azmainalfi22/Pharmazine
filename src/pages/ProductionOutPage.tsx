...
const ProductionOutPage = () => {
  ...
  const handleAddItem = () => {
    ...
    if (!newItem.production_order) {
      toast.error('Please enter production order number');
      return;
    }
    ...
    const newProductionItem: ProductionItem = {
      ...
      production_order: newItem.production_order,
      notes: newItem.notes
    };

    setProductionItems([...productionItems, newProductionItem]);
  ...
  const handleRemoveItem = (itemId: string) => {
    setProductionItems(productionItems.filter(item => item.id !== itemId));
  };

  const handleSubmitProduction = async () => {
    if (productionItems.length === 0) {
      toast.error('Please add at least one item for production');
      return;
    }
    ...
    try {
      for (const item of productionItems) {
        await apiClient.createStockTransaction({
          ...
          unit_price: 0,
          notes: `Production Order: ${item.production_order}. ${item.notes}`,
          reason: 'Production Consumption'
        });
      }

      toast.success('Production out completed successfully');
      setProductionItems([]);
      setProductionOrder('');
      setNotes('');
      fetchData(); // Refresh product data
    } catch (error) {
      console.error('Error processing production out:', error);
      toast.error('Failed to process production out');
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-teal-600 via-emerald-600 to-teal-700 p-8 rounded-2xl border-2 border-teal-200/20 shadow-2xl">
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-3xl font-bold text-white drop-shadow-lg mb-2">Production Out</h1>
            <p className="text-white/90 text-base">Stock consumed for production</p>
          </div>
          <Button 
            variant="outline" 
            onClick={fetchData} 
      ...
      <div className="bg-card border-2 rounded-lg overflow-hidden mb-12 shadow-lg">
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 dark:from-teal-950/20 dark:to-emerald-950/20 p-4 border-b-2 border-teal-200 dark:border-teal-800">
          <h2 className="text-lg font-bold text-teal-900 dark:text-teal-100">Production Information</h2>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  setProductionOrder(e.target.value);
                  setNewItem({...newItem, production_order: e.target.value});
                }}
              />
            </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-teal-200 dark:border-teal-800 hover:bg-teal-50 dark:hover:bg-teal-950/20">
                ...
                <TableHead className="text-teal-900 dark:text-teal-100 font-semibold text-right">Notes</TableHead>
              ...
              <TableRow>
                ...
                <TableCell className="text-right">
                  <Button 
                    ...
                    disabled={!selectedProduct || newItem.quantity <= 0 || !newItem.production_order}
      ...
      {productionItems.length > 0 && (
        <div className="bg-card border-2 rounded-lg overflow-hidden mb-12 shadow-lg">
          <div className="bg-gradient-to-r from-teal-50 to-emerald-50 dark:from-teal-950/20 dark:to-emerald-950/20 p-4 border-b-2 border-teal-200 dark:border-teal-800">
            <h2 className="text-lg font-bold text-teal-900 dark:text-teal-100">Items for Production ({productionItems.length})</h2>
          </div>
...
export default ProductionOutPage;