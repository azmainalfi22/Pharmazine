...
export class ApiClient {
  ...
  async createRequisition(payload: {
    store_id?: string;
    items: { product_id: string; qty: number; unit?: string }[];
  }) {
  ...
  async recordSalePayment(
    ...
    return this.fetch(`/sales/${saleId}/payment`, {
      method: "POST",
      body: JSON.stringify(payload),
    });
  }
  ...
  async downloadTemplate(
    kind: "products" | "suppliers" | "customers" | "opening_stock"
  ): Promise<Blob> {
  ...
  async downloadImportTemplate(
    type: "products" | "suppliers" | "customers" | "opening-stock"
  ) {
    const typeMap: Record<
      string,
      "products" | "suppliers" | "customers" | "opening_stock"
    > = {
      products: "products",
      suppliers: "suppliers",
      customers: "customers",
  ...
  async registerUser(userData: RegisterRequest): Promise<Profile> {
    return this.fetch<Profile>("/auth/register", {
      method: "POST",
      body: JSON.stringify({
        ...userData,
  ...
  async createStockTransaction(
    transaction: Partial<StockTransaction>
  ...
  async generatePOFromReorder(
    supplierId: string,
    days: number = 30
  ): Promise<any> {
    return this.fetch<any>(
      `/auto-reorder/generate-po/${supplierId}?days=${days}`,
      {
        method: "POST",
      }
    );
  ...
  async getPatientMedicationHistory(
    ...
    return this.fetch<any>(
      `/patients/${customerId}/medication-history?days=${days}`
    );
  }
  ...
  async getRefillReminders(daysAhead: number = 7): Promise<any> {
    return this.fetch<any>(
      `/patients/refill-reminders?days_ahead=${daysAhead}`
    );
  }
  ...
  async getAutoReorderLog(status?: string): Promise<any> {
    const url = status
      ? `/auto-reorder/log?status=${status}`
      : "/auto-reorder/log";
    return this.fetch<any>(url);
  }
  ...
  async createPOFromReorderLog(logId: string): Promise<any> {
    return this.fetch<any>(`/auto-reorder/log/${logId}/create-po`, {
      method: "POST",
    });
  }
  ...
  async createMasterReceive(
    receive: MasterReceiveCreate
  ): Promise<MasterReceive> {
  ...
  async createReceiveDetails(
    detail: ReceiveDetailsCreate
  ...
  async createOpeningStock(
    ...
    return this.fetch<StockManagementResponse>(
